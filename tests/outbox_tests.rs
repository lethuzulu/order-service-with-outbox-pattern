...
async fn setup() -> Db {
    ...
    sqlx::query("DELETE FROM outbox_messages").execute(&db.pool).await.unwrap();
...
async fn create_order_writes_both_rows_atomically() {
    ...
    let order_id = db
        .create_order(customer(), ten_cents())
        .await
        .unwrap();
...
async fn the_dual_write_gap_is_visible_naively_and_closed_by_outbox() {
    ...
    assert_eq!(pending_before, 0, "no outbox message — the fulfilment service never hears about this order");
    ...
    let _: Result<_, _> = db
        .with_transaction(|mut tx| async move {
            ...
            Err::<((), sqlx::Transaction<'_, sqlx::Postgres>), OutboxError>(OutboxError::Config("simulated crash".into()))
        })
        .await;
    ...
    assert_eq!(orders_after,  0, "with outbox pattern: order does not exist after failure");
    assert_eq!(pending_after, 0, "with outbox pattern: the gap cannot exist");
}
...
async fn poller_publishes_order_message_with_correct_payload() {
    ...
    let amqp_url = std::env::var("AMQP_URL")
        .expect("AMQP_URL required for this test");
    ...
    let handle = tokio::spawn(async move {
        fast_poller(poller_db).run(pub_ref, shutdown_rx).await
    });
    ...
    let order = db.get_order(order_id).await.unwrap().unwrap();
    assert_eq!(order.amount.cents(), 1000, "order amount unchanged by poller");
}
...
async fn concurrent_pollers_never_claim_same_message() {
    ...
    for _ in 0..10 {
        db.create_order(customer(), ten_cents())
            .await
            .unwrap();
    }
...
async fn publish_failure_marks_message_failed_and_leaves_order_intact() {
    ...
    let order = db.get_order(order_id).await.unwrap().unwrap();
    assert_eq!(order.amount.cents(), 1000, "order amount unchanged by publish failure");
    assert_eq!(order.status.to_string(), "pending", "order status unchanged");
}
